...
module parameters
  ...
  integer, parameter :: dimX = 256
  integer, parameter :: dimV = 256
...
!~   integer, parameter, dimension(3) :: sizev = [2*(dimV(1)), 1, 1]
  ...
  real(kind=DTYPE), parameter :: xe = 1.
  real(kind=DTYPE), parameter :: xb = 0.
  
!~   real(kind=DTYPE), parameter, dimension(3) :: xe = [2, 2, 2]
!~   real(kind=DTYPE), parameter, dimension(3) :: xb = [-2, -2, -2]
  
  real(kind=DTYPE), parameter :: ve = 1.
  real(kind=DTYPE), parameter :: vb = 0.
  ...
  real(kind=DTYPE), parameter :: CFL = 0.5
